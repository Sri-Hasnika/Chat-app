        ...
        {mediaFiles?.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 animate-fade-in">
            {mediaFiles.map((file, index) => (
              <div key={index} className="overflow-hidden rounded-lg shadow-lg bg-white transform hover:scale-105 transition-transform duration-300 ease-in-out" >
                <a href={file.image} target="_blank" rel="noopener noreferrer">
                  <img src={file.image} alt={`Media ${index + 1}`} className="w-full h-48 object-cover" />
                </a>
              </div>
            ))}
          </div>
